...
uniform vec4 u_resolution;   // screen size in pixels
uniform vec4 u_mouse;		 // mouse location (XY) and click (ZW) in pixels
uniform float u_time;		 // elapsed program time in seconds


// Adapted from Descent 3D by mhnewman: https://www.shadertoy.com/view/wdfGW4
...
vec3 addSign(vec3 color, vec3 pos, float side, vec2 id) {
    ...
    float charV = -3.5 + 8.0 * noise(id + 6.0 * charPos);
    charPos = fract(charPos);
    charV *= smoothstep(0.0, 0.4, charPos.x) * smoothstep(1.0, 0.6, charPos.x);
    charV *= smoothstep(0.0, 0.4, charPos.y) * smoothstep(1.0, 0.6, charPos.y);
    color = mix(color, signColor, flash * flicker * step(outline, 0.01) * clamp(charV, 0.0, 1.0));
